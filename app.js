...
const getFollowingPeopeleIdsOfUser = async username => {
  const getFlowwingPoepleQuery = `
select following_user_id from follower
inner join user on follower.following_user_id = user.user_id
where user.username = '${username}';
`
  const result = await db.all(getFlowwingPoepleQuery)
  const arrayOfUser = result.map(eachArray => eachArray.following_user_id)
  return arrayOfUser
}
...
const authenticateToken = (request, response, next) => {
  ...
  if (jwtToken === undefined) {
    ...
    jwt.verify(jwtToken, 'sriram', async (error, payload) => {
      if (error) {
        response.status(401)
        response.send('Invalid JWT Token')
      } else {
...
app.post('/login/', async (request, response) => {
  ...
  if (dbUser === undefined) {
    ...
    if (isPasswordMatched === true) {
      ...
      const jwtToken = jwt.sign(payload, 'sriram')
...
app.get('/user/tweets/feed/', authenticateToken, async (request, response) => {
  ...
  const followingPeopleId = await getFollowingPeopeleIdsOfUser(username)
  // const {limit} = request.query
  const selectQuery = `
     select username, tweet as tweet, date_time as dateTime from user inner join tweet on user.user_id=tweet.user_id
...
app.get('/user/following/', authenticateToken, async (request, response) => {
  ...
  `
  const followingPeople = await db.all(getUserNames)
...
app.get('/user/followers/', authenticateToken, async (request, response) => {
  const {username, userId} = request
  const getUserNames = `
  select DISTINCT name from follower inner join user on user.user_id = follower.following_user_id
  where follower_user_id = '${userId}'
  `
  const followingPeople = await db.all(getUserNames)
  response.send(followingPeople)
